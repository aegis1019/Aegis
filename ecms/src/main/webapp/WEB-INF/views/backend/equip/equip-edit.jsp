...
<head>
    ...
    <script type="Text/javascript">
        ...
        function CheckData() {
            ...
            if ($.trim($('#tbEquipmentName').val()) == "") {
                $.messager.alert(GNAMESPACE.Common.AlertMsgTitle, "设备名称不能为空！", 'info');
                return false;
            }

            if ($.trim($('#tbEquipmentNumber').val()) == "") {
                $.messager.alert(GNAMESPACE.Common.AlertMsgTitle, "计划数量不能为空！", 'info');
                return false;
            }
        ...
        function Save() {
            ...
            objFormData.EquipmentID = EquipmentJson.EquipmentID;
            $.messager.progress({ title: GNAMESPACE.Common.ProgressMsgTitle, msg: '正在保存数据。' });
            $.ajax({
                type: "POST",
                url: '/equipment/SaveForEdit',
...
<body class="FormBody2">
    ...
    <div id="divInputArea">
        ...
        <div class="e-row">
            ...
            <div class="e-col4 Field">
                <input id="tbEquipmentNumber" class="Field1" type="text" />
            </div>
        </div>
         <div class="e-row">
